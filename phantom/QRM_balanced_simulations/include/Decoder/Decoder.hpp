...
public:
    ...
    void f_plus_quaternary(const vector<double>* p_fst, const vector<double>* p_snd, 
        const int size, vector<double>* p_new);
    void f_minus_quaternary(const vector<double>* p_fst, const vector<double>* p_snd, 
        const vector<vector<int>>& u, const int size, vector<double>* p_new);
    double phi_depolarize(const double& mu, const vector<double>& p, const int& u1, const int& v1);
};
