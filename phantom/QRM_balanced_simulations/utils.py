...
class MeasurementCircuit:
    ...
    def add_MX(self, index):
        self.MX.append(index)
    def add_MZ(self, index):
        self.MZ.append(index)
    def propagate(self, pauli_string):
        # clean Pauli string to identity at qubits being measured
        x, z = pauli_string.to_numpy()
        for index in self.MX + self.MZ:
            x[index] = 0
            z[index] = 0
        return stim.PauliString.from_numpy(xs=x, zs=z)
...
def propagate(
    ...
    for circuit in circuits:
        if type(circuit) is MeasurementCircuit:
            pauli_string = circuit.propagate(pauli_string)
        else:
...
def pauli_string_is_all_equal(arr):
...
def format_pauli_string(s):
    return f"weight={s.weight} -- X: {s.pauli_indices('X')}, Y: {s.pauli_indices('Y')}, Z: {s.pauli_indices('Z')}"
...
# decompose any permutation into at most two layers of involution (SWAP)
# first decompose the permutation into disjoint cycles
# then write every even or odd cycle as a product of two involutions:
# (1,...,2n)   = [(n,n+1) (n-1,n+2) ... (1,2n)] * [(n,n+2) (n-1,n+3) ... (2,2n)]
# (1,...,2n+1) = [(n,n+1) (n-1,n+2) ... (1,2n)] * [(n,n+2) (n-1,n+3) ... (1,2n+1)]
#                |----------- trans1 ---------| * |----------- trans2 -----------|
def involutions_for_cycle_length(m):
    """
    Given the length m of a cycle (1..m), return two lists of transpositions
    (using 1-based indices) representing involutions A, B such that:
        (1 2 ... m) = B ∘ A
    """
    trans1 = []
    trans2 = []
    ...
    if m == 1:
        # Trivial cycle: identity
        return trans1, trans2

    if m == 2:
        # Simple transposition: (1 2) = (1 2) ∘ id
        return [(1, 2)], []

    if m % 2 == 0: # even length, m = 2n
        n = m // 2
        # A = (n,n+1) (n-1,n+2) ... (1,2n)
        for a in range(n, 0, -1):
            b = m + 1 - a  
            trans1.append((a, b))

        # B = (n,n+2) (n-1,n+3) ... (2,2n)
        for a in range(n, 1, -1):
            b = m + 2 - a
            trans2.append((a, b))
    else:          # odd lendth, m = 2n+1
        n = (m - 1) // 2
        # A = (n,n+1) (n-1,n+2) ... (1,2n)
        for a in range(n, 0, -1):
            b = m - a 
...
def decompose_into_two_involutions(perm):
    """
    Decompose a permutation into two layers of SWAPs (involutions).

    Input:
        perm: list of length n, 0-based, where perm[i] is the image of i.

    Output:
        swaps1, swaps2: lists of pairs (i, j) with 0-based indices such that:
            - Each list describes an involution (product of disjoint transpositions).
            - If I1 is obtained by applying swaps1 to identity, and I2 from swaps2,
              then perm == I2 ∘ I1.
    """
    n = len(perm)
    visited = [False] * n
    ...
    for i in range(n):
        ...
        # Get the two involutions for a cycle of length m on labels 1..m
        local_t1, local_t2 = involutions_for_cycle_length(m)

        # Map them to actual indices (0-based) using cycle[k-1]
        for a, b in local_t1:
            swaps1.append((cycle[a - 1], cycle[b - 1]))
        for a, b in local_t2:
            swaps2.append((cycle[a - 1], cycle[b - 1]))
