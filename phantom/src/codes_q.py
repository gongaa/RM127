...
class css_code(): # a refactored version of Roffe's package
    ...
    def __init__(self, hx=np.array([[]]), hz=np.array([[]]), code_distance=np.nan, name=None, name_prefix="", check_css=False):
        ...
        self.lx_hx = np.vstack((self.lx, self.hx))
        self.lz_hz = np.vstack((self.lz, self.hz))

    def is_X_stabilizer(self, op):
        return not ((self.hx_perp @ op.T) % 2).any() # product all zero

    def is_Z_stabilizer(self, op):
        return not ((self.hz_perp @ op.T) % 2).any() # product all zero

    def is_X_logical(self, op):
        if self.is_X_stabilizer(op):
            print("is X stabilizer, i.e., not X logical")
            return False
        if not ((self.hz @ op.T) % 2).any():
            ok, coeff = gf2_solve(self.lx_hx.T, op)
            assert ok
            return (True, coeff[:self.K] % 2)
    
    
    def is_Z_logical(self, op):
        if self.is_Z_stabilizer(op):
            print("is Z stabilizer, i.e., not Z logical")
            return False
        if not ((self.hx @ op.T) % 2).any():
            ok, coeff = gf2_solve(self.lz_hz.T, op)
            assert ok
            return (True, coeff[:self.K] % 2)
    ...
    def compute_logicals(self):
        ...
        self.lz = inverse(commutation).T @ self.lz % 2

        return self.lx, self.lz

    def set_X_logicals(self, new_lx):
        self.lx = new_lx
        commutation = self.lx @ self.lz.T % 2
        self.lz = inverse(commutation).T @ self.lz % 2
        self.lx_hx = np.vstack((self.lx, self.hx))
...
def gf2_rref(M):
    ...
    r, c = 0, 0
    pivots = []
    while r < M.shape[0] and c < M.shape[1]:
        # find pivot
        p = np.argmax(M[r:, c]) + r
        if M[p, c] == 0:
            c += 1
...
def gf2_solve(A, b):
    """Solve A^T x^T = b^T over GF(2); returns (has_solution, one_solution_x)."""
    A = A.copy() % 2
    b = b.copy() % 2
    # Build augmented matrix [A | b]
    Ab = np.concatenate([A, b[:, None]], axis=1)
    R, pivots = gf2_rref(Ab)
    # Check consistency: any row [0 ... 0 | 1]?
    if np.any((R[:, :-1].sum(axis=1) % 2 == 0) & (R[:, -1] == 1)):
        return False, None
    # Back-substitute to extract one solution.
    m, n = A.shape
    ...
    for r in reversed(range(m)):
        ...
        if not row.any():
            continue
        c = np.argmax(row)  # leftmost 1 in RREF row
        rest = (row ^ (row[c] * np.eye(1, n, c, dtype=np.uint8)[0]))  # row with pivot cleared
        sum_rest = (rest & x).sum() % 2
        x[c] = (R[r, -1] ^ sum_rest) % 2
    return True, x
...
def poly_div_inner(f, g): # over GF(2)
    if len(f) < len(g):
        return poly_div_inner(g, f)
    q = [0] * (len(f)-len(g)+1)
...
def poly_div(f_coeff, g_coeff):
    return poly2coeff(poly_div_inner(coeff2poly(f_coeff), coeff2poly(g_coeff)))
...
def get_primitive_polynomial(m):
    ...
    if m == 2:
        ...
        primitive_polynomial = 0b10000011
    elif m == 8:
