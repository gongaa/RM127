...
Decoder_polar_SCL::Decoder_polar_SCL(const int& K, const int& N, const int& L, const vector<bool>& frozen_bits, const vector<int>& info_indices) 
: Decoder(K, N), L(L), frozen_bits(frozen_bits), info_indices(info_indices)
{
...
void Decoder_polar_SCL::get_info_bits(vector<bool>& Y_K)
{
    auto& path_leaves = leaves_array[best_path];
	int i = 0;
	for (int idx : info_indices) 
		Y_K[i++] = path_leaves[idx]->get_c()->s[0];
}
