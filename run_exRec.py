...
p_CNOT = 0.001
rec_type = "H"
num_batch_dict = {0.0005: 1000, 0.0008: 1000, 0.001: 1000, 0.002: 500, 0.003:48, 0.004:2}
rec_type_dict = {"H": "Hadamard", "S": "S", "CNOT": "CNOT", "T": "code_switch"}
parent_dir = "logs_exRec"

def run_exp(factor, factor_single, factor_correction, p_CNOT, rec_type):

    d1 = "SPAM_equal_CNOT" if factor == 1.0 else "SPAM_half_CNOT"
    d2 = rec_type_dict[rec_type]
    log_file = f"s{int(10*factor_single)}_c{int(10*factor_correction)}_p{str(p_CNOT).split('.')[1]}.log"
    ...
    # print(cmd, dest)
    process = subprocess.Popen(['sbatch', '--mem-per-cpu', '2000', '--time', '4:00:00', '--output', dest, '--wrap', cmd])

for factor in [0.5,1.0]:
    for factor_correction in [0.0, 1.0]:
        # for p_CNOT in [0.0005, 0.0008, 0.001, 0.002, 0.003, 0.004]:
        for p_CNOT in [0.003]:
            # for rec_type in ["H", "S", "CNOT", "T"]:
            for rec_type in ["CNOT"]:
                run_exp(factor, factor_single, factor_correction, p_CNOT, rec_type)
